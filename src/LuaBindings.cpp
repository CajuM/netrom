...
extern "C" {
typedef std::shared_ptr<netrom::GameObject> GameObject;
typedef std::shared_ptr<netrom::UIElement> UIElement;
typedef std::shared_ptr<netrom::Scene> Scene;
typedef std::shared_ptr<netrom::MsgBox> MsgBox;
typedef std::shared_ptr<netrom::GlyphMat> GlyphMat;
typedef netrom::Level Level;
typedef netrom::Netrom Netrom;
typedef struct {
	int x;
	int y;
} V2;
...
GlyphMat* GlyphMat_new(char** a, int h) {
	return (new GlyphMat(new netrom::GlyphMat(a, h)));
}
...
GameObject* GameObject_new(Level* level, GlyphMat** icon, GlyphMat** mask,
	...
	return (new GameObject(new netrom::GameObject(level, nicon, nmask)));
}
...
GameObject * Level_getNearGO(Level * self, GameObject * go) {
	return new GameObject(self->getNearGO(*go));
}

GameObject * Level_getGOByPos(Level * self, int x, int y) {
	return new GameObject(self->getGOByPos(x, y));
}
...
Scene* Scene_new(Level* level) {
	return (new Scene(new netrom::Scene(level)));
}
...
MsgBox* MsgBox_new(Level *level) {
	return (new MsgBox(new netrom::MsgBox(level)));
}
...
void UIElement_gc(UIElement* self) {
	delete self;
}
